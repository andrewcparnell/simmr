...
Can plot by group number of name:
```{r, fig.width = 8, fig.height = 6}
plot(geese_simmr, group_num = 1:8)
```
...
summary(geese_simmr_out, type = 'diagnostics',
        group_name = 'Day 1')
```
If the model has converged all values should be close to 1


Check that the model fitted well:
```{r, fig.width = 8, fig.height = 6, results = 'hide'}
posterior_predictive(geese_simmr_out, group_num = 5)
```
...
```{r, fig.width = 8, fig.height = 6}
prior_viz(geese_simmr_out, group_num = 2)
```

Look at the histogram of the dietary proportions:
```{r, fig.width = 8, fig.height = 6}
plot(geese_simmr_out, group_name = 'Day 93', type = 'histogram')
```

Compare different groups
```{r, fig.width = 8, fig.height = 6}
compare_groups(geese_simmr_out, group_num = c(1,4,6), 
