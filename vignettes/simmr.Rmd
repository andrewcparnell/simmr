...
We assume that if you have got this far you have [installed R](https://www.r-project.org). We also recommend installing [Rstudio](https://rstudio.com) as this provides a very neat interface to use R and `simmr`. The instructions below all assume you are using Rstudio.
...
To combine more than two sources, simply run `combine_sources` recursively over the sources you want to combine. 
...
Note that the above actually changes the x-axis and not the x-axis despite the above command. This is because the coordinates are flipped in the ggplot. 
