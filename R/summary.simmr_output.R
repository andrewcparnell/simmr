summary.simmr_output = function(object, type = c("diagnostics", "quantiles", "statistics", 
    ...
    for (i in 1:length(group)) {
        ...
        out_quantiles[[i]] = t(apply(out_all, 2, "quantile", probs = c(0.025, 
            0.25, 0.5, 0.75, 0.975)))
    ...
    if (object$input$n_groups == 1) {
        invisible(list(gelman = out_bgr[[1]], quantiles = out_quantiles[[1]], 
            statistics = out_statistics[[1]], correlations = out_cor[[1]]))
    } else {
        invisible(list(gelman = out_bgr, quantiles = out_quantiles, statistics = out_statistics, 
            correlations = out_cor))
    }
