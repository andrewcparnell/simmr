...
g=ggplot(data=df,
  ...
  geom_pointrange(aes(x=x,y=y,ymax=y_upper,ymin=y_lower,height=0.2,shape=Source),size=0.7) +
  scale_shape_manual(values=1:nlevels(df$Source)) +
  theme(legend.title=element_blank(),legend.key = element_blank()) +
  guides(color=guide_legend(override.aes=list(linetype=c(0,rep(1,x$n_sources))))) +
