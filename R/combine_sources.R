combine_sources = function(simmr_out, to_combine = simmr_out$input$source_names[1:2], 
    ...
    simmr_new_out$input$source_means[to_combine_cols[1], ] = apply(old_source_means[to_combine_cols, 
        ], 2, "mean")
    ...
    simmr_new_out$input$source_sds[to_combine_cols[1], ] = apply(old_source_sds[to_combine_cols, 
    ...
    simmr_new_out$input$correction_means[to_combine_cols[1], ] = apply(old_correction_means[to_combine_cols, 
        ], 2, "mean")
    ...
    simmr_new_out$input$correction_sds[to_combine_cols[1], ] = apply(old_correction_sds[to_combine_cols, 
        ], 2, function(x) sqrt(sum(x^2)))
    ...
    simmr_new_out$input$concentration_means[to_combine_cols[1], ] = apply(old_concentration_means[to_combine_cols, 
        ], 2, "mean")
    ...
    for (j in 1:length(old_pars)) {
        for (i in 1:length(old_pars[[j]])) {
            ...
            sum_post = old_pars[[j]][[i]][, to_combine_cols[1]] + old_pars[[j]][[i]][, 
                to_combine_cols[2]]
            simmr_new_out$output[[j]][[i]][, to_combine_cols[1]] = sum_post
            colnames(simmr_new_out$output[[j]][[i]]) = c(simmr_new_out$input$source_names, 
