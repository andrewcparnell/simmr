...
if(!is.null(concentration_means)) {
  ...
  if(any(concentration_means== 0)) stop("Invalid values in concentration_means; must all be non-zero")
...
if(!is.null(group)) {
  ...
  if(min(group_int)!=1) stop("Group integers must start at 1")
  if(!all(diff(sort(unique(group_int)))==1)) stop("Group integers must proceed sequentially from 1, 2, 3, etc")
