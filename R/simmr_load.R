simmr_load = function(mixtures, source_names, source_means, source_sds, correction_means = NULL, 
    correction_sds = NULL, concentration_means = NULL, group = NULL) {
    
    # Function to load in data for simmr and check whether it's appropriate for
    # running through simmr_mcmc
    
    # Go through each object and check that it matches the requirements
    
    # Mixtures must be a matrix - the number of rows is the number of observations
    # and the number of columns is the number of tracers
    ...
    # source_names must be a character vector - the length of it is the number of
    # sources
    ...
    # source_means and source_sds must both be matrices where the number of rows
    # is n_sources (in the same order as source_names) and the number of columns
    # is n_tracers
    ...
    if (is.null(correction_means) & !is.null(correction_sds)) 
        stop("Both correction_means and correction_sds must be supplied")
    if (!is.null(correction_means) & is.null(correction_sds)) 
        stop("Both correction_means and correction_sds must be supplied")
    
    # correction_means and correction_sds must be matrix and of same dimension as
    # n_sources
    ...
    if (!is.null(group)) {
        if (!is.integer(group)) 
            stop("group variable needs to be of integer type. Perhaps use as.integer?")
        if (min(group) != 1) 
            stop("Group number must start at 1")
        if (!all(diff(sort(unique(group))) == 1)) 
            stop("Group numbers must proceed sequentially from 1, 2, 3, etc")
    ...
    out = list(mixtures = mixtures, source_names = source_names, source_means = source_means, 
        source_sds = source_sds, correction_means = correction_means, correction_sds = correction_sds, 
        concentration_means = concentration_means, group = group, n_obs = n_obs, 
        n_tracers = n_tracers, n_sources = n_sources, n_groups = n_groups)
    
    # Look through to see whether there are any missing values in anything
    if (any(unlist(lapply(out, "is.na")))) 
        stop("Missing values provided for some values. Check your inputs")
