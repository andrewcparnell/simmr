...
#' (CLR). The \code{\link{simmr_elicit}} function allows the user to specify
...
#'
#' This function takes the desired proportion means and standard deviations,
...
#' \code{n_sims} value.
#'
#' @param x An object of class \code{simmr_input} created using
...
#' @seealso See \code{\link{simmr_load}} to create an object suitable for input
#' into this function.
#'
...
simmr_elicit <-
  function(x,
           proportion_means = rep(0, x$n_sources),
           proportion_sds = rep(0.1, x$n_sources),
           n_sims = 1000) {
    # x must be a simmr_input xect
    if (class(x) != 'simmr_input')
      stop("Input argument x must have come from simmr_load")
    # proportion_means must be a vector of length x$n_sources
    if (length(proportion_means) != x$n_sources)
    ...
    if (length(proportion_sds) != x$n_sources)
    ...
    opt_mean = stats::optim(c(rep(0, (x$n_sources - 1))), clr_opt_mean, method =
                              "SANN")
    ...
    clr_opt_sd = function(pars) {
      ...
      # Generate n_sims observations from this normal distribution
      f = matrix(
        stats::rnorm(n_sims * x$n_sources, mean = best_mean, sd = sd),
        ncol = x$n_sources,
        nrow = n_sims,
        byrow = TRUE
      )
    ...
    best_f = matrix(
      stats::rnorm(n_sims * x$n_sources, mean = best_mean, sd = best_sd),
      ncol = x$n_sources,
      nrow = n_sims,
      byrow = TRUE
    )
