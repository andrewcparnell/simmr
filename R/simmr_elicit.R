simmr_elicit <- function(x, proportion_means = rep(0, x$n_sources), proportion_sds = rep(0.1, 
    x$n_sources), n_sims = 1000) {
    ...
    # Perform an optimisation to match the standard deviations to a normal
    # distribution
    clr_opt_mean = function(pars) {
        ...
        f = matrix(stats::rnorm(n_sims * x$n_sources, mean = means, sd = rep(1, 
            x$n_sources)), ncol = x$n_sources, nrow = n_sims, byrow = TRUE)
    ...
    clr_opt_sd = function(pars) {
        ...
        # Generate n_sims observations from this normal distribution
        f = matrix(stats::rnorm(n_sims * x$n_sources, mean = best_mean, sd = sd), 
            ncol = x$n_sources, nrow = n_sims, byrow = TRUE)
        p = as.matrix(compositions::clrInv(f))
    ...
    best_f = matrix(stats::rnorm(n_sims * x$n_sources, mean = best_mean, sd = best_sd), 
        ncol = x$n_sources, nrow = n_sims, byrow = TRUE)
